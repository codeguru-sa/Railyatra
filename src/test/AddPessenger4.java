...
import java.io.*;
import java.sql.*;
public class AddPessenger4 extends HttpServlet{
...
HttpSession hs = req.getSession(false);
		...
		ps.setLong(1, tno);
			...
			if(rs.next()){
				...
				pw.println("select{mrgin:40px; width:500px;  height:50px;	padding:0 20px; "
				...
				pw.println("<form action='book6' method='post'>");
				...
				pw.println("<select name='pmode'><option value=-1>Select Payment Mode</option>"
						+ "<option value='debit'>Debit Card</option>"
						+ "<option value='cradit'>Cradit Card</option>"
						+ "<option value='paytm'>Paytm</option>"
						+ "<option value='bhim'>Bhim App</option>"
						+ "<option value='phonepay'>Phone  Pay</option>"
						+ "</select>");
				pw.println("<br><br><br>");
				pw.println("<input type='submit' value='Procced To  Pay'>");
